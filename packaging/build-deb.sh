#!/usr/bin/env bash
# Сборка минимального .deb: бинарник в /usr/lib/ozon_radar/bin, скрипты в /usr/lib/ozon_radar/scripts,
# symlink /usr/bin/ozon_radar (так находится ozon_fetch.py относительно каталога exe).
set -euo pipefail
...
mkdir -p "$STAGE/DEBIAN" \
  "$STAGE/usr/lib/ozon_radar/bin" \
  "$STAGE/usr/lib/ozon_radar/scripts" \
  "$STAGE/usr/bin"

install -m755 "$BUILD/ozon_radar" "$STAGE/usr/lib/ozon_radar/bin/ozon_radar"
for f in "$ROOT"/scripts/*.py; do
  install -m644 "$f" "$STAGE/usr/lib/ozon_radar/scripts/$(basename "$f")"
done
ln -sf ../lib/ozon_radar/bin/ozon_radar "$STAGE/usr/bin/ozon_radar"
...
Description: OzonRadar — Qt UI и Python-загрузчик страниц
